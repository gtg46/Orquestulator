...
async def evaluate_expression(
    ...
    query_type: str = Path(
        ...,
        description="The type of expression to evaluate",
        regex="^(yaql|jinja2|orquesta)$",
    ),
    ...
    try:
        if query_type == "yaql":
            ...
            orquesta_data["__state"] = {
                ...
                "sequence": [{"status": task_status, "result": task_result}],
        ...
        return EvaluateResponse(result=result, query_type=query_type)
        ...
        raise HTTPException(
            ...
            detail=ErrorResponse(
                error=f"Template security violation: {str(e)}", query_type=query_type
            ).dict(),
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=ErrorResponse(
                error=f"Template error: {str(e)}", query_type=query_type
            ).dict(),
        )
